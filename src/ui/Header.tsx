...
export function Header({ mode, onMode, onShuffle }: HeaderProps) {
  return (
    <header className='z-[3] flex items-center justify-between gap-2 border-b border-line bg-white px-[14px] py-[10px] md:px-10 md:py-4'>
      <div className='grid grid-cols-[auto_1fr] items-center gap-x-2 font-display text-[19px] font-extrabold tracking-[-.02em] text-navy md:gap-x-3 md:text-2xl'>
        <span className={BRICK} aria-hidden='true' />
        <span>Minifigs</span>
        <small className='col-start-2 -mt-0.5 font-sans text-[10px] font-medium whitespace-nowrap text-muted md:text-xs'>
